...
class Comments extends MY_Controller {
    function __construct() {
        ...
        $this->load->model('bp_model/Discussions_model');
        $this->load->model('bp_model/Comments_model');
    ...
    public function index() {
        ...
        if ($this->form_validation->run() == FALSE) { 
        	...
        	$page_data['title'] = "Comment";
        	$page_data['heading'] = "";
        	$page_data['main'] = 'blueprints/comments/view';
            ...
            if ($this->Comments_model->new_comment($data)) {
                redirect('blueprints/comments/index/'.$ds_id);
            } else {
                // error
    ...
    public function flag() {
        ...
        if ($this->Comments_model->flag($cm_id)) {
            redirect('blueprints/comments/index/'.$this->uri->segment(3));
        } else {
            // error
            // load view and flash sess error
        }        
